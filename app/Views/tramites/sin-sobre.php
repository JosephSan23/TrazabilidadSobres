...
<link
    rel="stylesheet"
    href="<?= $escape(Url::to('/css/sin-sobre.css')) ?>">
...
<section class="custodia-page">
    <div class="custodia-page__header">
        <div>
            <h1>Gestión de Trámites y Custodia</h1>
            <p>
                Filtro y emisión técnica de fichas de custodia para sobres físicos recepcionados.
            </p>
        </div>
    </div>
    ...
    <div class="custodia-card">
        <div class="custodia-card__body">
            <!-- Formulario de Filtros con botón de Filtrar integrado -->
            <form
                method="get"
            ...
            <!-- Formulario envolvente para las acciones en lote y la tabla -->
            <form
                method="post"
                ...
                <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-3">
                        <button
                            ...
                            disabled>
                            Generar fichas de custodia (<span data-selected-count>0</span>)
                        </button>
                        <p class="mb-0 text-muted">
                            <strong><?= $escape($totalTramites) ?></strong> trámite(s) pendiente(s) de enlazar.
                        </p>
                    </div>

                    <?php if ($totalTramites > 0): ?>
                        <small class="text-muted">
                            Rango: <?= $escape($firstRecord) ?> - <?= $escape($lastRecord) ?> / <?= $escape($totalTramites) ?> 
                            | Página <?= $escape($currentPage) ?> de <?= $escape($totalPages) ?>
                        </small>
                    <?php endif; ?>
                </div>
                            ...
                            <select class="form-select form-select-sm" required>
                                <?php foreach ($ubicaciones as $ubicacion): ?>
                                    <option
                                        ...
                                        <?= $ubicacion['codigo'] === 'PEND-UBICAR' ? 'selected' : '' ?>>
                                        <?= $escape($ubicacion['codigo']) ?> - <?= $escape($ubicacion['nombre']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </template>
                            ...
                            <select
                                ...
                                <?php foreach ($usuarios as $usuario): ?>
                                    <option value="<?= $escape($usuario['id_usuario']) ?>">
                                        <?= $escape($usuario['nombre_completo']) ?> - <?= $escape($usuario['nombre_rol']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                            ...
                            <tbody>
                                <?php foreach ($tramites as $tramite): ?>
                                    <tr>
                                        ...
                                        <td><?= $escape($tramite['fec_tramite'] ?? 'Sin fecha registrada') ?></td>
                                        <td>
                                            <span class="badge bg-light text-dark border">
                                                <?= $escape($tramite['nombre_estado'] ?? 'Estado no identificado') ?>
                                            </span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
...
<script
    src="<?= $escape(Url::to('/js/tramites-pendientes.js', ['v' => '2'])) ?>"></script>